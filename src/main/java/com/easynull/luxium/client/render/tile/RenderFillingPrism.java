...
public class RenderFillingPrism extends LuxiumRender<TileFillingPrism> {
    ...
    public void render(TileFillingPrism prism, float partialTicks, PoseStack ps, MultiBufferSource source, int light, int overlay) {
        ...
        if(!stack.isEmpty()){
            ps.pushPose();
            ps.translate(0.5F, 1.1F + (float)(Math.sin(Math.toRadians(ticksUp)) * 0.83125F), 0.5F);
            ...
            renderModel(stack, ItemTransforms.TransformType.FIXED, ps, source, light, OverlayTexture.NO_OVERLAY);
            ps.popPose();
        }
    }
}
